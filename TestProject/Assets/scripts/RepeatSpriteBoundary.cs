...
public class RepeatSpriteBoundary : MonoBehaviour {
	...
	void Awake () {
		...
		for (int i = 1, l = (int)Mathf.Round(sprite.bounds.size.x); i < l; i++) {
			for (int j = 1, m = (int)Mathf.Round (sprite.bounds.size.y); j < m; j++) {

				child = Instantiate (childPrefab) as GameObject;
				child.transform.position = transform.position - (new Vector3 (spriteSize.x, 0, 0) * j) - (new Vector3 (0, spriteSize.y, 0)*i);
				child.transform.parent = transform;
			}
		}
